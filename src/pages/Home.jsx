...
export default function Home({ homeData, companyInfo, servicesData }) {
  const getIcon = (index) => {
    switch (index) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <section className="container features-container">
        <h2 className="sr-only">清潔維護團隊特色</h2>
          ...
          <div className="grid-2" style={{ alignItems: 'center' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                ...
                <a href="/services" className="btn btn-outline">了解服務</a>
              </div>
            </div>
              ...
              {coreProjects.map((project, index) => (
                ...
                >
                  ...
                  <div style={{ position: 'absolute', left: '1rem', right: '1rem', bottom: '1rem', color: '#fff' }}>
                    ...
                    <h3 style={{ margin: '0.2rem 0 0', color: '#fff', fontSize: '1rem', lineHeight: 1.35 }}>{project.title}</h3>
                  </div>
                </article>
              ))}
            </div>
        ...
        <section className="section-padding bg-light" id="services-overview">
          <div className="container">
            ...
            <div className="grid-3" style={{ gap: '2rem' }}>
              {servicesData.items.slice(0, 3).map((item) => (
                <div key={item.id} className="home-service-card animate-fade-in" style={{ display: 'flex', flexDirection: 'column', backgroundColor: 'white', borderRadius: 'var(--radius-md)', overflow: 'hidden', boxShadow: 'var(--shadow-sm)', border: '1px solid #f1f5f9' }}>
                  <SiteImage src={getServiceImage(item)} alt={item.title} sizes="(max-width: 768px) 100vw, 400px" style={{ width: '100%', height: '200px', objectFit: 'cover' }} />
                  <div style={{ padding: '2rem', flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
                    ...
                    <a href={`/services#${item.id}`} className="btn btn-outline" style={{ width: '100%', fontSize: '0.85rem', padding: '0.5rem 1rem', marginTop: 'auto' }}>
                      <span>了解詳細細項</span>
            ...
            <div style={{ textAlign: 'center', marginTop: '3rem' }}>
              <a href="/services" className="btn btn-primary" style={{ padding: '0.75rem 2.5rem' }}>
                查看所有服務項目
              </a>
            </div>
          </div>
      ...
      <section className="section-padding" style={{ backgroundColor: 'var(--bg-white)', borderBottom: '1px solid #f1f5f9' }}>
        <div className="container" style={{ textAlign: 'center', maxWidth: '800px' }}>
          ...
          <div style={{ display: 'flex', gap: '1.25rem', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href={`tel:${(companyInfo.phoneFormatted || companyInfo.phone).replace(/[^+\d]/g, '')}`} className="btn btn-primary" style={{ padding: '0.85rem 2rem' }}>
...
// Render the actual published figures in the initial HTML as well.
